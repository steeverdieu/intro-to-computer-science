...
Consider this following function prototype in c:
```
int sum( int a, int b );
```
...
## Syntactic sugar
Syntactic sugar are syntaxes in programming languages that are designed to make things easier to read or to express.

Syntactic sugar make the expressions shorter or more concise.

Let say you have a variable called `counter`, and you want to add 1 to this variable, the basic way to do this is like below:
```
counter = counter + 1
```
There are also orther ways to express that. We call these orther syntaxes `syntactic sugar` :
1. `counter += 1`
2. `counter++`
3. `++counter`
...
By example, in the expression : `a < b`, the symbol `<` is a logical operator. It is the symbol used to compare the value `a` to `b`.

Here is a list of the most used logical operators :
- `==` used to compare if two values are equal
- `<` used to verify if a value is smaller than another one
- `>` used to verify if a value is bigger than another one
- `<=` used to verify if a value is smaller or equal to another one
- `>=` used to verify if a value is bigger or equal to another one

## How to evaluate the quality of a code
What one individual values as top quality code may be drastically different from what another individual values. That means that the quality of a code can depend on the needs of the people or team working on the project, the problem that the code solves or the programming language used. But when we evaluate the quality of a code, there are three main aspects we have to always consider :  
